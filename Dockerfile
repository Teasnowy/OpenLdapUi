...
RUN echo 'server  {' > /etc/nginx/http.d/openldapui.conf && \
    ...
    echo '    location ~ .*\.(gif|jpg|jpeg|png|bmp|swf)$' >> /etc/nginx/http.d/openldapui.conf && \
    echo '    {' >> /etc/nginx/http.d/openldapui.conf && \
    echo '      expires      30d;' >> /etc/nginx/http.d/openldapui.conf && \
    echo '    }' >> /etc/nginx/http.d/openldapui.conf && \
    echo '' >> /etc/nginx/http.d/openldapui.conf && \
    echo '    location / {' >> /etc/nginx/http.d/openldapui.conf && \
    echo '        try_files $uri $uri/ /index.html;' >> /etc/nginx/http.d/openldapui.conf && \
    echo '    }' >> /etc/nginx/http.d/openldapui.conf && \
    echo '' >> /etc/nginx/http.d/openldapui.conf && \
